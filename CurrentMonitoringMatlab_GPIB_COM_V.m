...
File2Load={}%'1-30-13_2.txt', '1-30-13_1.txt', '1-31-13_1.txt', '1-31-13_2.txt', '1-31-13_3.txt'};
%------------------------------------------------------------------
%ITP_20mMTrHp_100mMTrHCl_0.1Trit_Blood_5.txt
%------------------------------------------------------------------
FileName='2-8-13.txt';
...
if ~isempty(File2Load)
    ...
    figure(1);
    subplot(2,1,1);
    plot(A(:,1),A(:,3),'r','linewidth',2); hold on;
    end % for
end
...
fprintf(g,':SOUR:FUNC:MODE VOLT');   % Set voltage sourcing
fprintf(g,':SOUR:VOLT:MODE FIX');    % Set fixed voltage value mode
%fprintf(g,':SOUR:VOLT:RANG 500.0'); % Set voltage output range
fprintf(g,':SOUR:VOLT:RANG:AUTO 1'); % Enable automatic voltage range
fprintf(g,[':SOUR:VOLT ',num2str(V)]); % Set output voltage  (immediate update, does not wait for trigger)
